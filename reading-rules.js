...
(() => {
  ...
  function makeReport(chart) {
    ...
    add('4. 금전운','구조적 단서',[
      ...
      `${wealth.count>=3?`재성 ${wealth.element}이 보이는 글자에 ${wealth.count}번 나타납니다. 수입 기회를 넓히는 선택과 현금 흐름을 지키는 선택을 별도로 평가해보세요.`:wealth.count===0?'재성이 보이는 여덟 글자에는 나타나지 않습니다. 일이나 투자 가능성을 배제하지 말고 지장간 및 실제 경력을 따로 확인해야 합니다.':`재성 ${wealth.element}이 ${wealth.count}번 보입니다. 수입의 많고 적음보다 정기 수입과 지출을 확인하는 질문부터 시작하세요.`} 원국의 재성 표시 개수는 부채나 수익의 확률이 아닙니다.`,
    ...
    add('5. 직업운','구조적 단서',[
      ...
      `${career.count>output.count?`보이는 글자에서는 관성 ${career.count}개가 식상 ${output.count}개보다 많습니다. 역할과 평가 기준이 분명한 업무에서 책임의 부담이 커지는지 살펴보세요.`:output.count>career.count?`보이는 글자에서는 식상 ${output.count}개가 관성 ${career.count}개보다 많습니다. 직접 만든 결과물을 설명하고 전달하는 업무가 잘 맞았는지 실제 경력으로 확인하세요.`:'관성과 식상이 보이는 글자에서 같은 개수입니다. 직무의 규칙과 결과물의 자율성 중 어느 쪽이 성과를 낳았는지 실제 경험으로 구분하세요.'} 이것만으로 조직형·창업형 적성을 판정하지 않습니다.`,
    ...
    add('6. 연애운','관계 점검 질문',[
      ...
      `일지 ${day.branch}의 오행 ${day.branchElement}은 일간 ${day.stemElement}과 ${labels[relation(day.stemElement,day.branchElement)]} 관계입니다. 이를 일상 언어로 바꾸면 ${({동료:'서로의 독립성과 거리',표현:'감정과 의견을 표현하는 방식',재물:'시간과 비용을 나누는 방식',책임:'기대와 약속의 기준',지원:'서로 돕고 의지하는 방식'})[relation(day.stemElement,day.branchElement)]}에 관한 대화를 해볼 수 있습니다. 실제 연애 성향을 확인하는 검사가 아닙니다.`,
